...
`compose.yaml` levanta `postgis/postgis:16-3.5` en **127.0.0.1:5434**. En la primera creación del volumen, aplica automáticamente `db/init/001_schema.sql` y `db/init/002_seed.sql`: activa PostGIS, crea `lugares`, el índice GIST e inserta tres puntos de ejemplo.

Si usas una instalación local de PostgreSQL + PostGIS, crea una base `geoportal`, ejecuta esos dos archivos SQL en ese orden como usuario con permisos y ajusta `.env` a tu host, puerto y credenciales. Por ejemplo:

```powershell
psql -U postgres -d geoportal -f .\db\init\001_schema.sql
psql -U postgres -d geoportal -f .\db\init\002_seed.sql
```
...
Los puntos se colorean por categoría. La búsqueda por radio trabaja respecto del **centro actual del mapa**. El mapa base necesita acceso a Internet; la API puede probarse por terminal sin ese mapa.
